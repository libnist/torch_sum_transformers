...
class CNNBehindFnetModel(nn.Module):
    def __init__(self,
                 d_model: int,
                 num_layers: int,
                 num_cnn_blocks: int,
    ...
    def forward(self, 
        ...
        encoder_output = self.encoder(embedded_inputs)
        
        compressed_encoder_output = self.cnn_block(encoder_output)
        
        attn_mask = get_attn_mask(sum_tokens.shape[-1],
                                  device=sum_tokens.device)
        decoder_output = self.decoder(embedded_outputs,
                                      compressed_encoder_output,
                                      attn_mask)
